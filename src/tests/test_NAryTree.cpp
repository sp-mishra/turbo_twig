...
TEST_CASE("[NArayTree] Flatten preorder flat hierarchy") {
    using IntTree = NAryTree<int>;
    IntTree tree;
    auto *root = tree.insert(nullptr, 1);
    auto *c1 = tree.insert(root, 2);
    ...
    auto fh = akriti::tree::layout_extras::flatten_preorder(tree, tree.get_root());

    // order by node data should match preorder (deterministic)
    REQUIRE(fh.order.size() == 4);
    std::vector<int> data_order;
    for (auto node_ptr : fh.order) data_order.push_back(node_ptr->data);
    REQUIRE(data_order == std::vector<int>{1, 2, 4, 3});

    // alias for npos
    const std::size_t npos = decltype(fh)::npos;

    // parents (indices into fh.order)
    REQUIRE(fh.parent.size() == 4);
    REQUIRE(fh.parent[0] == npos); // root has no parent
    REQUIRE(fh.parent[1] == 0);    // '2' child of '1'
    REQUIRE(fh.parent[2] == 1);    // '4' child of '2'
    REQUIRE(fh.parent[3] == 0);    // '3' child of '1'

    // first_child
    REQUIRE(fh.first_child[0] == 1); // first child of '1' is '2'
    REQUIRE(fh.first_child[1] == 2); // first child of '2' is '4'
    REQUIRE(fh.first_child[2] == npos);
    REQUIRE(fh.first_child[3] == npos);

    // next_sibling
    REQUIRE(fh.next_sibling[0] == npos);
    REQUIRE(fh.next_sibling[1] == 3); // sibling of '2' is '3'
    REQUIRE(fh.next_sibling[2] == npos);
    REQUIRE(fh.next_sibling[3] == npos);

    // subtree ranges [begin, end)
    REQUIRE(fh.subtree_begin[0] == 0);
    REQUIRE(fh.subtree_end[0] == 4); // entire tree
    REQUIRE(fh.subtree_begin[1] == 1);
    REQUIRE(fh.subtree_end[1] == 3); // '2' and '4'
    REQUIRE(fh.subtree_begin[2] == 2);
    REQUIRE(fh.subtree_end[2] == 3); // '4'
    REQUIRE(fh.subtree_begin[3] == 3);
    REQUIRE(fh.subtree_end[3] == 4); // '3'
}
